...
typedef struct HumanPoint {
    ...
    HumanPoint() { x = 0.0; y = 0.0; z = 0.0; cell_width=0.3; cell_height=0.4; id=0; type=0; vx=0; vy =0;}
    HumanPoint(double xx, double yy) : x(xx), y(yy) { z = 0.0; cell_width=0.3; cell_height=0.4; id=0; type=0; vx=0; vy =0;}
    HumanPoint(double xx, double yy, double zz, double ii, double tt) : x(xx), y(yy), z(zz), id(ii), type(tt) { cell_width=0.3; cell_height=0.4; vx=0; vy =0;}
    HumanPoint(double* p) { x = p[0]; y = p[1]; z = p[2]; id=p[3]; type=p[4]; cell_width=0.3; cell_height=0.4; vx=0; vy =0;}
    HumanPoint(double xx, double yy, double zz, double ii, double tt, double vxx, double vyy) : x(xx), y(yy), z(zz), id(ii), type(tt), vx(vxx), vy(vyy) {
     cell_width=0.3; cell_height=0.4; 
 }
