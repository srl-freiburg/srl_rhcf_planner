...
#include <fstream>
#include <iostream>
#include <srl_rhcf_planner/srl_trajectory.h>
...
#include <srl_rhcf_planner/world_model.h>
#include <srl_rhcf_planner/costmap_model.h>
...
#include <srl_rhcf_planner/smp/components/samplers/theta_star_in_regions.hpp>
#include <srl_rhcf_planner/smp/components/distance_evaluators/kdtree.hpp>
#include <srl_rhcf_planner/smp/components/extenders/pos.hpp>
...
#include <srl_rhcf_planner/smp/components/collision_checkers/collisionCostMap.hpp>
// #include <srl_global_planner/smp/components/collision_checkers/rectangle.hpp>

#include <srl_rhcf_planner/smp/components/multipurpose/minimum_time_reachability_thetastar.hpp>
#include <srl_rhcf_planner/smp/planners/rrtstar.hpp>
#include <srl_rhcf_planner/smp/planner_utils/trajectory.hpp>
#include <srl_rhcf_planner/smp/utils/branch_and_bound_euclidean.hpp>
...
class Srl_rhcf_planner : public nav_core::BaseGlobalPlanner
{
...
private:
    ...
    ros::Publisher pub_obstacleMask_;
...
public:
    ...
    */
    Srl_rhcf_planner() : costmap_ros_(NULL), initialized_(false),  world_model_(NULL) {
    ...
    */
    Srl_rhcf_planner(std::string name, costmap_2d::Costmap2DROS* costmap_ros) : costmap_ros_(costmap_ros)
    {
    ...
    */
    ...
    /**
    * @brief Set Obstacle mask used to constraint RRT in a homotopy class
    * @return void
    */
    void setObstacleMask(std::vector< geometry_msgs::PoseStamped > plan);

    /**
    * @brief Restore the original values in the global cost map
    * @return void
    */
    void restoreCostMap();
    ...
    std::vector<Tobstacle> obstacle_positions_mask; ///<  @brief Obstacles
    ...
    double b_local_planner_;

    bool** obstacleMask_;

    double obstacleMask_resolution_;

    double obstacleMask_origin_x_;

    double obstacleMask_origin_y_;

    int obstacleMask_width_;

    int obstacleMask_height_;

    int buffer_mask_;
    
    bool publish_mask_markers_;

    int** initialCostMap_;

    bool initialCostFilled_;

    bool stopMarkingAgents_ ;
