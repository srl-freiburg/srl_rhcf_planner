...
#include <srl_rhcf_planner/smp/planner_utils/trajectory.h>
#include <srl_rhcf_planner/smp/planner_utils/vertex_edge.h>

/// Costant used to constraint the trajectories to the homotopy classes.
/// In RRT* every time a trajectory is checked for collision also its 
/// cost is evaluated. If one of the state is far from the discrete path
/// the NO_HOMOTOPY_COST is added, therefore a trajectory with a higher cost
/// is discarded, see rrtstar.h line #102

#define NO_HOMOTOPY_COST 100000
...
namespace smp {
    ...
    */
    ...
    public:
        ...
        virtual double check_band (state_t *state_initial_in,trajectory_t *trajectory_in, state_t *state_final_in = 0 ) = 0 ;
