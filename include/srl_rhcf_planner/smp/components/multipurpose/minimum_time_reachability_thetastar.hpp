...
#include <srl_rhcf_planner/smp/components/multipurpose/minimum_time_reachability_thetastar.h>

#include <srl_rhcf_planner/smp/planners/rrtstar.hpp>
#include <srl_rhcf_planner/smp/common/region.hpp>
#include <srl_rhcf_planner/smp/components/model_checkers/base.hpp>
#include <srl_rhcf_planner/smp/components/cost_evaluators/base.hpp>
...
/// check if the trajectory is not far from the discrete path

template< class typeparams, int NUM_DIMENSIONS >
double smp::minimum_time_reachability<typeparams,NUM_DIMENSIONS>
::check_band (state_t *state_initial_in,
    ...
    for (typename list<state_t*>::iterator iter=trajectory_in->list_states.begin();
              ...
              for(int j=0;j<N-1;j++){
                ...
                relativ_dist(j,0)=sqrt((traj_x-sx_)*(traj_x-sx_)+(traj_y-sy_)*(traj_y-sy_));
              ...
              if(d > 2*width_thetastar_tunnel_)
                { 
                  // ROS_WARN(" d: %f , bound %f", d, 2*width_thetastar_tunnel_);
                  return 0;
                }
...
::cost_proj (double x, double y, double theta){
    ...
    if(MODEL_COST==2){
      ...
      if(d>width_thetastar_tunnel_){
          res += NO_HOMOTOPY_COST;
        }
