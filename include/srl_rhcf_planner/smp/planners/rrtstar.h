...
#include <srl_rhcf_planner/smp/planners/base_incremental.hpp>
#include <srl_rhcf_planner/smp/planners/planner_parameters.h>
    ...
    */
    ...
    private:
        ...
        int check_extended_trajectory_for_collision (state_t *state, trajectory_t *trajectory) {
            ...
            /// Checking cost.. 
            /// avoiding trajectory which are far from the discrete path
            double cost_traj = this->cost_evaluator.check_band (state, trajectory);

            trajectory->list_states.pop_front ();


            if(cost_traj<1) // TODO: change this into a global param
                {   
                    return 0;
                }
            else
